...
Download the tarball of stable release from [Libxc website](https://tddft.org/programs/libxc/) or [gitlab](https://gitlab.com/libxc/libxc/-/releases). For example, download `libxc-6.1.0` to [Perlmutter](https://docs.nersc.gov/systems/perlmutter/):


```bash
wget https://gitlab.com/libxc/libxc/-/archive/6.1.0/libxc-6.1.0.tar.gz
```

Extract the tarball and follow the [instruction](https://tddft.org/programs/libxc/installation/) to configure and build Libxc

```bash
./configure --prefix=/global/common/software/<project>/libxc
make
make check
make install
```
